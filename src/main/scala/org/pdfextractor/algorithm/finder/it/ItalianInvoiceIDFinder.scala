...
class ItalianInvoiceIDFinder extends AbstractFinder {
  ...
  private[it] var PATTERN_ITALIAN_INVOICE_ID_START_BARE_START_PART_AS_REGEX
    : Regex = _
  ...
  def refreshed(): Unit = {
    ...
    valuePattern = Some(
      ("(?i)" + phraseTypesStore.buildAllPhrases(
        ...
        INVOICE_ID) + """([\s]{0,})([:]{0,1})([\s]{0,})(\w{1,}[^\s]{0,})""").r)
    PATTERN_ITALIAN_INVOICE_ID_START_PART_AS_REGEX =
      ("(?i)" + phraseTypesStore.buildAllPhrases(
        SupportedLocales.ITALY,
        INVOICE_ID) + """([\s]{0,})([:]{0,1})([\s]{0,})""").r
  ...
  override def parseValue(raw: String, regex: Regex): Any = {
    var ret: String = {
      if (PATTERN_ITALIAN_INVOICE_ID_START_PART_AS_REGEX
            .findFirstIn(raw)
            .nonEmpty)
        PATTERN_ITALIAN_INVOICE_ID_START_PART_AS_REGEX.replaceFirstIn(raw, "")
      else if (PATTERN_ITALIAN_INVOICE_ID_START_BARE_START_PART_AS_REGEX
                 .findFirstIn(raw)
                 .nonEmpty)
        PATTERN_ITALIAN_INVOICE_ID_START_BARE_START_PART_AS_REGEX
          .replaceFirstIn(raw, "")
        ...
        throw new IllegalArgumentException(
          "No invoice id start was found: '" + raw + "'")
    }
